...
describe("Komponen RegisterForm", () => {
  ...
  test("harus menampilkan pesan error jika form dikirim dengan field wajib kosong", async () => {
    // jsdom 26 memblokir submit saat input required kosong (berbeda dgn CRA/jest
    // jsdom 16). fireEvent.submit memicu onSubmit langsung — perilaku yang diuji
    // (validasi custom -> pesan error) tetap sama.
    render(<RegisterForm />, { wrapper: TestWrapper });
    const registerButton = screen.getByRole("button", { name: /Register/i });
    fireEvent.submit(registerButton.form);
