...
// ============================================================
// ROADMAP #16: Refund endpoint + payment audit trail
// ============================================================
/**
 * Process refund for an order (seller/admin only).
 * POST /payment/refund
...
exports.refundPayment = async (req, res) => {
  ...
  try {
    ...
    const refundableStatuses = ["COMPLETED", "PAID"];
    if (!refundableStatuses.includes(order.order_status)) {
      return handleError(res, {
        statusCode: 400,
        message: `Pesanan tidak bisa di-refund (status: ${order.order_status}). Hanya status COMPLETED/PAID.`,
      });
    }
    ...
    const refundAmount = amount && amount > 0 && amount <= order.total_price
      ? amount
      : order.total_price;
...
exports.getPaymentAudit = async (req, res) => {
  ...
  try {
    ...
    if (orderError) throw orderError;
    if (!order) {
      return handleError(res, { statusCode: 404, message: "Pesanan tidak ditemukan." });
    }
