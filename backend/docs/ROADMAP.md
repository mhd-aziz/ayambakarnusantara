...
| 2026-08-29 | Fix temuan HIGH audit QA 2026-08-29: **HIGH-1** perbaikan `storageHelper.extractPathFromPublicUrl` agar menerima URL Supabase lengkap dan mengisolasi validasi path traversal (`..`, `//`, `/`) pada path hasil ekstrak + unit test di `security-fixes.test.js` mengimpor fungsi asli; **HIGH-2** pembersihan duplikasi deklarasi fungsi `refundPayment` dan `getPaymentAudit` di `paymentController.js`; **HIGH-4** stabilisasi `api.integration.test.js` dengan pemulihan `products.stock` di `resetDemo()` dan bypass rate limiter saat `NODE_ENV === "test"` di `authRoutes.js`. Unit tests 36/36 PASS, Integration tests 14/14 PASS. |
