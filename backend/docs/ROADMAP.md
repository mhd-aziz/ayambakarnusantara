...
| 10 | Upload cek MIME saja, tanpa magic bytes | `multerConfig.js` | **Selesai (23 Agu 2026)**. Tambah `validateImageMagicBytes` di `utils/imageValidation.js` (cek signature PNG/JPEG/GIF/WEBP asli). Diterapkan di `chatController.sendMessage` (+ storageHelper.uploadImage). Unit test `tests/new-fixes.test.js` (7 test PASS). |
