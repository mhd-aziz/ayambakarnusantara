...
| 2026-08-05 | **Frontend DIKEMBALIKAN ke source original Bootstrap** (tetap Vite): seluruh folder `src/` + `src/css/` = persis repo original (react-bootstrap + bootstrap.css), Tailwind & folder `ui/` DIBUANG. Adaptasi Vite: `vite.config.js` (esbuild jsx untuk `.js`), `index.html` root, env `process.env.REACT_APP_*` → `import.meta.env.VITE_*`, env 3-tier konsisten backend (`.env` + `.env.dev` + `.env.prod`), test Vitest (jest alias + vi.mock idiom). XSS fix chat dipertahankan. Build OK, 33/33 test PASS, tampilan = original |
